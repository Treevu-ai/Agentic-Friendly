...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      window.omelette?.writeFile(DC_STATE_FILE, JSON.stringify({ sections: state.sections })).catch((err) => {
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  const apply = React.useCallback(() => {
    ...
    saveT.current = setTimeout(() => {
      try { localStorage.setItem(tfKey, JSON.stringify(tf.current)); } catch (err) {
  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      console.warn('[DCViewport] No se pudo restaurar el viewport desde localStorage:', err.message);
    }
  ...
  const doExport = (kind) => {
    ...
    dcExport(cardRef.current, width, height, name, kind)
      .catch((err) => {
        ...
        // Notificación visible: toast temporal en el DOM
        const toast = document.createElement('div');
        toast.textContent = 'Error al exportar el artboard. Revisa la consola para más detalles.';
        toast.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#c96442;color:#fff;padding:10px 18px;border-radius:8px;font-size:13px;z-index:99999;box-shadow:0 4px 16px rgba(0,0,0,.3);';
